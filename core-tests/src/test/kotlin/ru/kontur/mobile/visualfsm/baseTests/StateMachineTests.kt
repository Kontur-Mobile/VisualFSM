...
class StateMachineTests {
    ...
    @Test
    fun destroyFeatureAsyncCancellationTest() = runFSMFeatureTest(
        featureFactory = { dispatcher ->
            TestFSMFeature(TestFSMState.Initial, TestFSMAsyncWorker(dispatcher))
        }
    ) { feature, states ->
    ...
    private fun cancelByStartOtherAsyncTest() = runFSMFeatureTest(
        featureFactory = { dispatcher ->
            TestFSMFeature(
        ...
        feature.proceed(Start("async1", 2000))
        advanceTimeBy(100)

        feature.proceed(Start("async2", 2000))
        advanceTimeBy(1500)

        feature.proceed(Start("async3", 2000))
        advanceTimeBy(50)

        feature.proceed(Start("async4", 2000))
        advanceTimeBy(1999)

        feature.proceed(Start("async5", 2000))
        advanceUntilIdle()

        assertEquals(
            listOf(
                TestFSMState.Initial,
                TestFSMState.Async("async1", 2000),
                TestFSMState.Async("async2", 2000),
                TestFSMState.Async("async3", 2000),
                TestFSMState.Async("async4", 2000),
                TestFSMState.Async("async5", 2000),
