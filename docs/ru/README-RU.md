...
* AsyncWorkerTask.ExecuteIfNotExist - запустить, только если эквивалентная операция в данный момент не выполняется (
  приоритет выполняющейся операции с равным объектом состояния)
* AsyncWorkerTask.ExecuteIfNotExistWithSameClass - запустить, только если операция с тем же классом состояния в данный момент не выполняется (
  приоритет выполняющейся операции с тем же классом состояния, используется для операций с выдачей результата в несколько этапов)
...
- `VisualFSM.generateDigraph(...): String` - сгенерировать граф в DOT формате для визуализации в Graphviz (graphviz cli для CI или http://www.webgraphviz.com/ в браузере), используйте аргумент `useTransitionName` для подстановки имени `Transition` или `Action` класса в качестве имени ребра или аннотацию `@Edge("name")` для `Transition` класса чтобы установить произвольное имя ребра
