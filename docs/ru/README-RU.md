...
Для того, чтобы сгенерировать файл со всеми переходами необходимо в параметры ksp передать параметр `generateAllTransitionsFiles` со значением `true`.

```groovy
ksp {
    arg("generateAllTransitionsFiles", "true")
}
```

В пакет, в котором находится `Feature` сгенерируется файл с именем `[Имя базового State]AllTransitions.txt` со следующим содержимым:

```
[Имя базового State]
[Имя первого не sealed State]
...
[Пример](#AuthFSMStateAllTransitions.txt)
...
### AuthFSMStateAllTransitions.txt
...
ConnectionFailed,AsyncWorkState.Registering
```
